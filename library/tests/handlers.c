...
TEST(writev)
{
	...
	arg_types = SYSCALL_GET_ARG_TYPES(writev)(&env, __NR_writev, args);
	ASSERT_EQ(arg_types.arg_types[0].kind, IMMEDIATE);
	ASSERT_EQ(arg_types.arg_types[1].kind, POINTER);
	ASSERT_EQ(arg_types.arg_types[2].kind, IMMEDIATE);

	serialized[0] = serialize((const char *)&args[0], &arg_types.arg_types[0], &serialized_len[0]);
	ASSERT(serialized_len[0] > 0);
	ASSERT_NEQ(serialized[0], NULL);

	serialized[1] = serialize((const char *)&args[1], &arg_types.arg_types[1], &serialized_len[1]);
	ASSERT(serialized_len[1] > 0);
	ASSERT_NEQ(serialized[1], NULL);
	
	serialized[2] = serialize((const char *)&args[2], &arg_types.arg_types[2], &serialized_len[2]);
	ASSERT(serialized_len[2] > 0);
	ASSERT_NEQ(serialized[2], NULL);

	deserialize_in_place(serialized[0], &arg_types.arg_types[0], &deserialized_len[0]);
	ASSERT_EQ(deserialized_len[0], serialized_len[0]);
	ASSERT_EQ(*(int *)serialized[0], args[0]);

	deserialize_in_place(serialized[1], &arg_types.arg_types[1], &deserialized_len[1]);
	...
	deserialize_in_place(serialized[2], &arg_types.arg_types[2], &deserialized_len[2]);
